...
class Restructure_mdl():
    ...
    def extract_system_blk(self):
        ...
        with open(self._file, 'r') as file:
            ...
            for line in file:
                ...
                self.update_brace_count(tokens)
                ...
                if stack_pop_brace_count == self._brace_count:
                    val = stack.pop()
                    if val == "Block":
                        self.model_info.update_blk_info(blk_info)
                        blk_info = ''

                    elif val == "Line":
                        self.model_info.update_line_info(line_info)
                        line_info = ''
                    stack_pop_brace_count -= 1
                    if not stack:
                        try:
    ...
    def bfs_ordering_validation(self,path):
        ...
        mdl_info = model_info()
        _processed_output = []
        with open(path,'r') as bfs_file:
            stack.append("Model")
            for line in bfs_file:
                line = remove_extra_white_spaces(line)
                tokens = get_tokens(line)
                self.update_brace_count(tokens)
                ...
                if "Model" == tokens[0]:
                    _processed_output.append(line)
                    while get_tokens(line)[0] != "Name":
                        line = remove_extra_white_spaces(next(bfs_file))
                    _processed_output.append(line)
                elif tokens[0] == "System" and stack[-1] == "Model":
                    stack_pop_brace_count += 1
                    stack.append(tokens[0])
                ...
                if stack_pop_brace_count == self._brace_count:
                    ...
                    if val == "Block":
                        ...
                        line_info = ''
                    elif val == "System" and len(stack) == 1:
                    ...
                    if  len(stack) == 0 :
                        try:
                            while True:
                                next_line = remove_extra_white_spaces(next(bfs_file))
                                _processed_output.append(next_line)
                        except StopIteration:
                            break
                    elif stack[-1] == "Model":
                        _processed_output.append(line)
            #print('\n'.join(_processed_output))
            return _processed_output
